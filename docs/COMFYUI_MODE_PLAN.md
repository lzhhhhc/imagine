...
真实使用里，生成页把步数、CFG、批量这些几乎不动的采样参数和真正每次都要改的内容摊在同一层；而图生图工作流里不接采样器的识别词（CLIPTextEncode.text）又只能当「其他参数」藏起来。本轮只改默认暴露方式，不改提交协议：

- `suggest()` 不再建议步数、CFG、宽高、批量。这些值继续用工作流原值；需要时仍可在编辑器里手动绑定，手动绑定的校验和提交行为不变。
- 图里只有一个 `LoadImage.image` 标量时，导入即建议为「参考图片」。多个 LoadImage 仍不猜测，留给用户显式选择。
- 工作台主区域只放提示词和参考图。绑定到 `CLIPTextEncode.text` 的自定义参数也直接露出多行输入，不再藏进折叠区；步数、CFG、种子等收到「生成参数」折叠区。
