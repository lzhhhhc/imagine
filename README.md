...
图像修图、多引擎导演分镜与视频制作，以及统一的「罗德岛终端」视觉主题。
...
| 导演台 | 分步访谈自动写入可编辑分镜，人物/环境附图；Grok 与 Seedance 逐镜视频制作、查询、保存与分享 |
...
视频配置与绘图、润色 LLM、ComfyUI 分开保存。请按服务商文档明确选择 Grok 标准、Grok 1.5 或 Seedance 2.x Ark 协议，并填写支持该能力的模型 ID；应用不会按模型名猜协议。

导演访谈完成后自动写入上方「分镜」，每镜可编辑提示词、秒数、首尾帧。「开始视频制作」显示实际人物/环境参考图、帧图、每镜时长、画幅、分辨率和模型；每次确认提交一个镜头，结果作为独立片段保存，可打开与分享。本版不自动拼接视频。H3 与可灵保留提示词创作，尚未接入直接生成。

Grok 每镜 1–15 秒、参考图最多 7 张；首尾帧与参考图组合需 Grok 1.5。Seedance 2.x 每镜 4–15 秒、参考图最多 9 张，当前入口将参考图与首尾帧模式分开校验。参考图不被自动当首帧，超限或不支持的画幅会在提交前提示，不丢弃参数或切换接口。

导演 LLM 需支持图片输入。带人物/环境参考图生成首帧时，绘图连接需使用 multipart 修图协议与支持多图的模型；不支持时明确报错。视频任务编号保存在本机，停止等待不取消服务器制作；可从制作记录继续查询和下载。提交结果不明时先核对服务商后台，避免重复扣费。
...
- [DIRECTOR_PRODUCTION.md](docs/DIRECTOR_PRODUCTION.md) —— 导演分镜、素材传递、视频协议与交付验证
